...
#officials. Each of them was to independently record the list of voters
#and send it to the collector. In Siruseri, every one has a ID number and
#the list would only list the ID numbers of the voters and not their names.
#The officials were expected to arrange the ID numbers in ascending order in
#their lists.

#On receiving the lists, the Collector realised that there were discrepancies
#- the three lists were not identical. He decided to go with the majority.
#That is, he decided to construct the final list including only those ID
#numbers that appeared in at least 2 out of the 3 lists. For example if
...
#Input format
#The first line of the input contains 3 integers N1, N2 and N3. N1 is the
#number of voters in the first list, N2 is the number of voters in the second
#list and N3 is the number of voters in the third list. The next N1 lines
#(lines 2,...,N1+1) contain one positive integer each and describe the first
#list in ascending order. The following N2 lines (lines N1+2,...,N1+N2+1)
#describe the second list in ascending order and the final N3 lines
